...
export const createCheckoutSession = async (
  ...
  try {
    ...
    const session = await stripe.billingPortal.sessions.create({
      ...
      return_url: process.env.NEXT_PUBLIC_BILLING_RETURN_URL as string,
      flow_data: {
        type: "subscription_update_confirm",
        after_completion: {
          type: "redirect",
          redirect: {
            return_url: process.env.NEXT_PUBLIC_BILLING_SUCCESS_URL as string,
          },
        },
...
export const getUserCurrentPlan = async (userId: string) => {
  ...
  const availableLinks = plan.quota.LINKS;
  const currentLinks = linksCount;
  const usage = (currentLinks / availableLinks) * 100;
